...
typedef struct _pomo_state_data {
  pomo_state state;
} pomo_state_data;
...
const char* read_pomodoro_running(void) {
    if (isRunning) { return "POMODORO: RUNNING"; }
    else { return "POMODORO: STOPPED"; }
}
...
void set_master_pomodoro_state(uint8_t in_buflen, const void *in_data, uint8_t out_buflen, void *out_data) {
  pomo_state_data *state_data = (pomo_state_data*)out_data;
...
void keyboard_post_init_user(void) {
    ...
    transaction_register_rpc(SYNC_POMODORO_STATE, set_master_pomodoro_state);
}
...
void housekeeping_task_user(void) {
    if (is_keyboard_master()) {
        ...
        if (isRunning && timer_elapsed32(lastSync) > 500) {
          pomo_state_data state_data = {state};
          if (transaction_rpc_recv(SYNC_POMODORO_STATE, sizeof(pomo_state_data), &state_data)) {
            lastSync = timer_read32();
